...
python -u main.py --dataset amazon/amazon_office --model newrec --augment --maxlen 20 --train_dir newrec_aug
...
python -u main.py --dataset amazon/amazon_tool --transfer --model newrec --state_dict_path res/amazon/amazon_office/newrec_base/newrec.epoch=201.lr=0.001.layer=2.head=1.hidden=50.maxlen=200.pth --inference_only --mode test --topk 10